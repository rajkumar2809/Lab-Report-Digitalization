...
from . models import myfile as xmyfile
import PyPDF2
import glob
import os
...
def dashboard(request):
    ...
    user = request.session['username']
    who = 'teacher' if myuser.objects.get(Username = user).teacher== True else 'student'
    return render(request, 'dashboard.html', {'list': filelist, 'filestorage':filestorage, 'who': who})

def student ( request ) : 
    data = xmyfile . objects . all () 
    return render ( request , 'student.html', {'data' : data})

def studentdownload ( request,user, file ) : 
        file_path = 'media/{}/{}'.format(user, file)
...
def upload(request):
    if 'username' in request.session and request.method == 'POST':
        ...
        url = fs . url ( filename )
        print ( url )
        xmyfile ( user = user , title = request . POST['title'], \
            description = request . POST ['description'], url = url ,\
            size = fs . size( filename)  ) . save () 
...
def search ( request ) : 
    ...
    filestosearch = [ url for u, t, d, url , s, t in xmyfile . objects . all () ] 
    ...
    for file in filestosearch:
        ...
        if '.pdf' in file : 
            ...
            print("The word {} was found {} times in {}".format(search_word, search_word_count, file ))
            if search_word_count > 0 : 
                foundfile . append ( xmyfile . objects . get ( url = file ) )
        else:
            with open(file,'r') as f:
                contents = f.read()
                # print ( contents )
            if searchdata . lower ()  in contents . lower () :
                search_word_count = contents . count ( searchdata )
                print("The word {} was found {} times in {} ".format(searchdata, contents . count ( searchdata), file ))
                if search_word_count > 0 : 
                    foundfile . append ( xmyfile . objects . get ( url = file ) ) 


    return render ( request , 'student.html', {'data' : foundfile } )